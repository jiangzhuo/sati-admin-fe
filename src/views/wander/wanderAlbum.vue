<template>
  <div class="app-container">
    <div class="filter-container">
      <el-button style="margin-left: 10px;" type="primary" icon="el-icon-edit" @click="handleCreate">{{ $t('wanderAlbum.add') }}</el-button>
    </div>

    <el-table v-loading="listLoading" ref="dataTable" :data="wanderAlbumList" border fit highlight-current-row style="width: 100%;">
      <el-table-column :label="$t('wanderAlbum.id')" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.id }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('wanderAlbum.name')" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.name }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('wanderAlbum.description')" align="center">
        <template slot-scope="scope">
          <el-popover
      ...
      <el-table-column :label="$t('wanderAlbum.scenes')" align="center">
        <template slot-scope="scope">
          <el-tag v-for="scene in scope.row.scenes" :key="scene">{{ sceneMap[scene].name }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column :label="$t('wanderAlbum.author')" align="center">
        <template slot-scope="scope">
      ...
      <el-table-column :label="$t('wanderAlbum.copy')" align="center">
        <template slot-scope="scope">
          <el-popover
    ...
    <el-dialog :title="dialogStatus" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :model="temp" label-position="left" label-width="70px" style="width: 400px; margin-left:50px;">
        <el-form-item :label="$t('wanderAlbum.name')" prop="name">
          <el-input v-model="temp.name"/>
        </el-form-item>
        <el-form-item :label="$t('wanderAlbum.description')" prop="description">
          <el-input
        ...
        <el-form-item :label="$t('wanderAlbum.background')" prop="background">
          <el-upload
        ...
        <!--</el-form-item>-->
        <el-form-item :label="$t('wanderAlbum.price')" prop="name">
          <el-input v-model="temp.price"/>
        </el-form-item>
        <el-form-item :label="$t('wanderAlbum.scenes')" prop="scenes">
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="dialogFormVisible = false">{{ $t('wanderAlbum.cancel') }}</el-button>
        <el-button v-if="dialogStatus=='create'" type="primary" @click="createData">{{ $t('wanderAlbum.confirm') }}</el-button>
        <el-button v-else type="primary" @click="updateData">{{ $t('wanderAlbum.confirm') }}</el-button>
      </div>
    </el-dialog>
...
import WANDER_ALBUM_REVERT_DELETED from '@/graphqls/wanderAlbumRevertDeleted.graphql'
...
export default {
  ...
  methods: {
    ...
    async handleRevertDeleted(row) {
      ...
      await this.$apollo.mutate({
        // 查询语句
        mutation: WANDER_ALBUM_REVERT_DELETED,
        // 参数
        variables: {
          id: row.id
        }
