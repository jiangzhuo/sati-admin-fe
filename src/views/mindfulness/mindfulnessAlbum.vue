<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-button class="filter-item" style="margin-left: 10px;" type="primary" icon="el-icon-edit" @click="handleCreate">{{ $t('mindfulnessAlbum.add') }}</el-button>
    ...
    <el-dialog :title="dialogStatus" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :model="temp" label-position="left" label-width="70px" style="width: 400px; margin-left:50px;">
        ...
        <el-form-item :label="$t('mindfulnessAlbum.status')" prop="status">
...
import MINDFULNESS_ALBUM_SEARCH from '@/graphqls/mindfulnessAlbumSearch.graphql'
...
export default {
  ...
  methods: {
    ...
    async getList(status = 0) {
      ...
      const result = await this.$apollo.query({
        // 查询语句
        query: MINDFULNESS_ALBUM_SEARCH,
        // 参数
        variables: {
      ...
      const promises = result.data.searchMindfulnessAlbum.data.data.map((mindfulnessAlbum) => {
        return this.getUser(mindfulnessAlbum.author)
      })
      await Promise.all(promises)
      console.log(this.userMap)
      this.mindfulnessAlbumList = result.data.searchMindfulnessAlbum.data.data
      this.total = result.data.searchMindfulnessAlbum.data.total
