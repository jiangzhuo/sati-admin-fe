<template>
  <div class="app-container">
    <div class="filter-container">
      <el-input :placeholder="$t('natureAlbum.keyword')" v-model="listQuery.keyword" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter"/>
      ...
      <el-button class="filter-item" style="margin-left: 10px;" type="primary" icon="el-icon-edit" @click="handleCreate">{{ $t('natureAlbum.add') }}</el-button>
    ...
    <el-table v-loading="listLoading" ref="dataTable" :data="natureAlbumList" border fit highlight-current-row style="width: 100%;">
      <el-table-column :label="$t('natureAlbum.id')" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.id }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('natureAlbum.name')" align="center">
        <template slot-scope="scope">
          <span>{{ scope.row.name }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('natureAlbum.description')" align="center">
        <template slot-scope="scope">
          <el-popover
      ...
      <el-table-column :label="$t('natureAlbum.background')" align="center">
        <template slot-scope="scope">
      ...
      <el-table-column :label="$t('natureAlbum.price')" align="center">
        <template slot-scope="scope">
          <!--<el-tag v-for="pid in scope.row.productId" :key="pid">{{ pid }}</el-tag>-->
          <span>{{ scope.row.price }}</span>
        </template>
      </el-table-column>
      <el-table-column :label="$t('natureAlbum.scenes')" align="center">
        <template slot-scope="scope">
          <el-tag v-for="scene in scope.row.scenes" :key="scene">{{ sceneMap[scene].name }}</el-tag>
        </template>
      </el-table-column>
      <el-table-column :label="$t('natureAlbum.author')" align="center">
        <template slot-scope="scope">
      ...
      <el-table-column :label="$t('natureAlbum.copy')" align="center">
        <template slot-scope="scope">
          <el-popover
      ...
      <el-table-column :label="$t('natureAlbum.status')" align="center">
        <template slot-scope="scope">
      ...
      <el-table-column :label="$t('natureAlbum.updateTime')" align="center">
        <template slot-scope="scope">
      ...
      <el-table-column :label="$t('natureAlbum.actions')" align="center" width="230" class-name="small-padding fixed-width">
        <template slot-scope="scope">
          <el-button type="primary" size="small" icon="el-icon-edit" @click="handleUpdate(scope.row)">{{ $t('natureAlbum.edit') }}</el-button>
          <!--<el-button v-if="scope.row.status&0b1" type="info" size="small" icon="el-icon-delete" @click="handleRevertDeleted(scope.row)">{{ $t('natureAlbum.revert') }}</el-button>-->
          <!--<el-button v-else type="danger" size="small" icon="el-icon-delete" @click="handleDelete(scope.row)">{{ $t('natureAlbum.delete') }}</el-button>-->
        </template>
      </el-table-column>
    </el-table>
    ...
    <el-dialog :title="dialogStatus" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :model="temp" label-position="left" label-width="70px" style="width: 400px; margin-left:50px;">
        <el-form-item :label="$t('natureAlbum.name')" prop="name">
          <el-input v-model="temp.name"/>
        </el-form-item>
        <el-form-item :label="$t('natureAlbum.description')" prop="description">
          <el-input
        ...
        <el-form-item :label="$t('natureAlbum.background')" prop="background">
          <el-upload
        ...
        <!--</el-form-item>-->
        <el-form-item :label="$t('natureAlbum.price')" prop="name">
          <el-input v-model="temp.price"/>
        </el-form-item>
        <el-form-item :label="$t('natureAlbum.scenes')" prop="scenes">
        ...
        <el-form-item :label="$t('natureAlbum.status')" prop="status">
        ...
        <el-form-item :label="$t('natureAlbum.copy')" prop="copy">
          <el-input
            :autosize="{ minRows: 2, maxRows: 4}"
            v-model="temp.copy"
            type="textarea"
        ...
        <el-form-item v-show="false" :label="$t('natureAlbum.author')" prop="author">
      ...
      <div slot="footer" class="dialog-footer">
        <el-button @click="dialogFormVisible = false">{{ $t('natureAlbum.cancel') }}</el-button>
        <el-button v-if="dialogStatus=='create'" type="primary" @click="createData">{{ $t('natureAlbum.confirm') }}</el-button>
        <el-button v-else type="primary" @click="updateData">{{ $t('natureAlbum.confirm') }}</el-button>
      </div>
    </el-dialog>
...
import NATURE_ALBUM_SEARCH from '@/graphqls/natureAlbumSearch.graphql'
import SCENE_ALL from '@/graphqls/sceneAll.graphql'
import NATURE_ALBUM_UPDATE from '@/graphqls/natureAlbumUpdate.graphql'
import NATURE_ALBUM_DELETE from '@/graphqls/natureAlbumDelete.graphql'
import NATURE_ALBUM_REVERT_DELETED from '@/graphqls/natureAlbumRevertDeleted.graphql'
import NATURE_ALBUM_CREATE from '@/graphqls/natureAlbumCreate.graphql'
import USER_BY_ID from '@/graphqls/userById.graphql'

export default {
  name: 'NatureAlbumTable',
  data() {
    return {
  ...
  methods: {
    ...
    async getList(status = 0) {
      ...
      const promises = result.data.searchNatureAlbum.data.data.map((natureAlbum) => {
        console.log(natureAlbum)
        return this.getUser(natureAlbum.author)
      })
      await Promise.all(promises)
      console.log(this.userMap)
      this.natureAlbumList = result.data.searchNatureAlbum.data.data
      this.total = result.data.searchNatureAlbum.data.total
    ...
    async createData() {
      ...
      if (data.data.createNatureAlbum.code !== 200) {
        this.$notify({
          title: '失败',
          message: data.data.createNatureAlbum.message,
          type: 'error',
          duration: 2000
        })
      } else {
    ...
    async handleRevertDeleted(row) {
      ...
      await this.$apollo.mutate({
        // 查询语句
        mutation: NATURE_ALBUM_REVERT_DELETED,
        // 参数
        variables: {
          id: row.id
        }
